...
function CoachingStack() {
  return (
    <Stack.Navigator screenOptions={{ headerStyle: { backgroundColor: colors.primary }, headerTintColor: '#fff', headerTitleStyle: { fontWeight: 'bold' } }}>
      <Stack.Screen name="CoachingForm" component={CoachingFormScreen} options={{ title: 'Private Coaching' }} />
    </Stack.Navigator>
  );
}
    ...
    <Tab.Navigator
      screenOptions={({ route }) => ({
        tabBarIcon: ({ focused, color, size }) => {
          ...
          else if (route.name === 'Upload') iconName = focused ? 'videocam' : 'videocam-outline';
          else if (route.name === 'Coaching') iconName = focused ? 'trophy' : 'trophy-outline';
        ...
        tabBarStyle: { backgroundColor: colors.surface, borderTopColor: colors.border, borderTopWidth: 1, paddingBottom: 5, paddingTop: 5, height: 60 },
        tabBarLabelStyle: { fontSize: 12, fontWeight: '500' },
        headerShown: false,
      })}
    >
      <Tab.Screen name="Home" component={HomeScreen} options={{ title: 'Home' }} />
      <Tab.Screen name="Upload" component={UploadStack} options={{ title: 'Upload' }} />
      <Tab.Screen name="Coaching" component={CoachingStack} options={{ title: 'Coaching' }} />
